...
public class StreamsExercise {
    public static List<Integer> returnSquareRoot(List<Integer> numbers) {
        return numbers.stream().map(number -> (int) Math.round(Math.sqrt(number))).collect(Collectors.toList());
    }
    ...
    public static List<String> getDistinctLetters(List<String> names) {
        return Arrays.stream(String.join("", names).split("")).distinct().toList();
    }
    ...
    public static double getAverageAge(List<User> users) {
        return users.stream().mapToDouble(User::getAge).sum() / (double) users.size();
    }
    ...
    public static List<Integer> generateFirst10PrimeNumbers() {
       return IntStream.iterate(2, n -> n+1).filter(StreamsExercise::isPrime).limit(10).boxed().collect(Collectors.toList());
    }
